...
NOISE_PATTERNS = [
    ...
    r"^Hash Values for Production Files",
]
...
def parse_google_letter_hashes(raw_text: str) -> List[Tuple[str, str]]:
    ...
    def collect_hash(idx: int) -> Tuple[str, int]:
        """Collect hex-only lines after a filename until 128 hex chars (SHA-512)."""
        j = idx
        hexbuf = ""
        while j < n and len(hexbuf) < 128:
            ln = lines[j].strip()
            if not ln or is_noise_line(lines[j]):
                j += 1
                continue
    ...
    while i < n:
        ...
        if not filename:
            break
        sha512, i_after_hash = collect_hash(i_after_name)
        if sha512:
...
class DropLineEdit(QLineEdit):
    """QLineEdit that accepts drag-and-drop of files/folders from the OS."""
    file_dropped = pyqtSignal(str)

    def __init__(self, file_filter: str | None = None, accept_dirs: bool = False, parent=None):
        super().__init__(parent)
        self.setAcceptDrops(True)
        self._file_filter = file_filter
        self._accept_dirs = accept_dirs

    def _extract_path(self, mime: QMimeData) -> str | None:
        if not mime.hasUrls():
            return None
        url = mime.urls()[0]
        ...
        if not path:
            return None
        if self._accept_dirs and os.path.isdir(path):
            return path
        if os.path.isfile(path):
            if self._file_filter is None:
                return path
            if any(path.lower().endswith(ext) for ext in self._file_filter.split()):
                return path
        return None
...
class UnzipNestedGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Input path (folder or file) — supports drag-and-drop
        input_layout = QHBoxLayout()
        self.input_edit = DropLineEdit(file_filter=".zip .7z", accept_dirs=True)
        self.input_edit.setPlaceholderText("Drop a folder or archive here, or click Browse")
        self.input_edit.file_dropped.connect(self._on_input_dropped)
        ...
        # Google letter PDF — supports drag-and-drop
        google_letter_layout = QHBoxLayout()
        self.google_letter_edit = DropLineEdit(file_filter=".pdf")
        self.google_letter_edit.setPlaceholderText("Drop a PDF here, or click Browse")
    ...
    def _on_input_dropped(self, path: str):
